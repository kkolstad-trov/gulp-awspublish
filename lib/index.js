...
/**
 * Replaces windows separators (\) with S3 safe separator (/)
 * @param  {string} the file path
 *
 * @return {string} the S3 safe path
 * @api private
 */
...
Publisher.prototype.publish = function (headers, options) {
  ...
  return through.obj(function (file, enc, cb) {
    ...
    if (file.isBuffer()) {
      ...
      // get s3 headers
      _this.client.headFile(fixSeparators(file.s3.path), function(err, res) {
        if (err) return cb(err);
        ...
        // update: file are different
          ...
          _this.client.putBuffer(file.contents, fixSeparators(file.s3.path), file.s3.headers, function(err, res) {
            if (err) return cb(err);
...
Publisher.prototype.sync = function(prefix) {
  ...
  stream._transform = function(file, encoding, cb) {
    // replace windows separator (\) with  with s3 separator (/)
    newFiles[fixSeparators(file.s3.path)] = true;
    this.push(file);
    cb();
  };
